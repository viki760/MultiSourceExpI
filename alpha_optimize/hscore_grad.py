import numpy as np
import sys
import copy
sys.path.append("/home/viki/Codes/MultiSource/3/multi_source_exp/MultiSourceExp")
import util.loading as loading
from fg_train.fixed_f_transfer import transfer_fg
# from fg_train.fixed_f import fg
# from metrics.OTCE import OTCE
from metrics.H_score import Hscore


def minimize_f(f, alpha, lr=0.001, epsilon = 0.0001, num_iters = 1000):
    score = np.zeros(num_iters)
    for i in range(num_iters):
        grad = np.zeros_like(alpha)
        for j in range(len(alpha)):
            a_plus, a_minus = np.copy(alpha), np.copy(alpha)
            a_plus[j] += epsilon
            a_minus[j] -= epsilon
            grad[j] = (f(a_plus)-f(a_minus))/(2*epsilon)
        alpha += lr*grad
        score[i] = f(alpha)

    return alpha, score

def update_alpha(t_id, s_id, include_target=True):
    #! 这里 include_target 为 False 时的 alpha 和为 1 约束条件没有解决
    n_s_tasks = len(s_id)
    alpha = np.ones(n_s_tasks) / (n_s_tasks + 1)
    f = lambda a: Hscore(id_t = t_id, id_s = copy.deepcopy(s_id), alpha = a, include_target = include_target)
    alpha_opt, score_curve = minimize_f(f, alpha)
    return alpha_opt, score_curve

if __name__ == '__main__':
    a,s = update_alpha(0, [1,2], True)