...
# load data
def load_data(path, batch_size=None, id=0, t=0, istarget=False, shots=10):
    '''
    ...
    if t == 0:
        x = torch.from_numpy(np.load(
            path+"x"+str(id)+"_train.npy").transpose((0, 3, 1, 2))).to(torch.float32)
        y = torch.from_numpy(np.load(path+"y"+str(id)+"_train.npy"))
    else:
        x = torch.from_numpy(
            np.load(path+"x"+str(id)+"_test.npy").transpose((0, 3, 1, 2))).to(torch.float32)
    ...
    if istarget == True:
        sample = np.random.choice(x.__len__(), shots, replace=False)
        x, y = x[sample], y[sample]

    data = torch.utils.data.DataLoader(
        TensorDataset(x, y), batch_size, shuffle=True)
...
def load_model(path=None, id=0, t=1):
    ...
    if not path == None:
        model_f.load_state_dict(torch.load(
            path+'f_task_n_'+str(id)+'.pth', map_location=device))
        model_g.load_state_dict(torch.load(
            path+'g_task_n_'+str(id)+'.pth', map_location=device))
